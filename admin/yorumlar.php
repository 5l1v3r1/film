<?php
	...
	$yazisor=$db->prepare("SELECT * FROM yorum order by yorum_id DESC LIMIT $Goster,$Limit");
	$yazisor->execute();
	$satir_sayisi = $db->query("SELECT * FROM yorum WHERE 1")->rowCount();
 ?>
		<!-- MAIN -->
		<div class="main">
			<!-- MAIN CONTENT -->
					...
					<div class="row">
						<div class="col-md-12">
			                ...
			                while($yazicek=$yazisor->fetch(PDO::FETCH_ASSOC)) { $say++?>
											<tr>
												<td><?php echo $say ?></td>
												<td><?php echo $yazicek['yorum_isim'] ?></td>
												<td><?php echo $yazicek['yorum_icerik'] ?></td>
												<td><?php echo $yazicek['yorum_eposta'] ?></td>
												<td><?php echo $yazicek['yorum_tarih'] ?></td>
												<td><a href="islem/islem.php?yorum_id=<?php echo $yazicek['yorum_id']; ?>&yorumsil=ok"><span class="label label-danger">Sil</span></a></td>
											</tr>
											 <?php } ?>
										</tbody>
									</table>
